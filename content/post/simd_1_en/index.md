---
title: SIMD part 1
description: Let's explore the basics of SIMDs!
slug: simd_1_en
date: 2026-03-28
image: cover.png
coverHeight: m3
...
*Part 3: benchmarking, pushing to limits*\
*Part 4: Assembly x86, fun instructions*\
*Part 5: history, compatibility, availability*

## Problem

Imagine a situation when we need to operate on large amounts of data using very trivial operations, for example for each pixel on the screen we take an illumination value and multiply that by the albedo value.

![Traditional approach](images/img1.png)

Repeat that for each screen and you get 2 million multiplications, performed one by one in a row. Another example: we work with matrices and we need to sum or multiply them together. Product of 2 4x4 matrices, which are the most common in 3D applications, takes 64 multiplications and 48 additions. That's a lot!

![4x4 matrix multiplication](images/img4.png)

## What should we do?

Depending on our view of the problem we could go several paths:
...
Engineers knew about the helplessness of CPUs when it comes to 3D graphics. One matrix multiplication is pretty expensive alone, and for believable scenes you need to calculate thousands and millions of them per frame.

## What is SIMD?

SIMD stands for `Single Instruction Multiple Data`[^1]. It is a hardware mechanism that allows processors to perform same instruction (addition, multiplication, load, store, reciprocal, etc) not one at a time, but 4, 8 or even 16 in parallel, depending on the CPU architecture and supported features. This way at the cost of one multiplication you can perform whopping 16 multiplications on modern CPUs or 8 on more-or-less average ones.
...
In C++ SSE 128-bit operations have `_mm` previx\
...
Separated with underscore using snake_case, next goes the operation:
...
Next goes data type and layout:
...
SIMDs commonly support integer operations on top of float ones. `epi32` is broken down into `epi` (`extended packed integer`) and 32 which means 32-bit integer.
...
Another very common suffix is `u` and it stands for *alignment*[^13]:
```cpp
_mm_load_ps   // aligned
...
a4 = _mm_max_ps( a4, b4 );
c4 = _mm_min_ps( a4, b4 );
__m256 e5 = _mm_set1_ps(3.14f);   // set all 8 floats to 3.14, aka broadcast it over all lanes
```
...
It was my first time really using SIMDs, and I did it wrong, here is how:

For the G-Buffer (all data I collect each frame - albedo, light, normals, distances, etc) I have so-called *Array Of Structures*, or AoS[^16]. This is the simplest and most naive approach, which makes my data layout linear:
```
x y z x y z x y z
```
...
Which is not only just ugly, but is also slow and loses the power of SIMD.

First issue is no data locality[^15]: when the order of memory access is linear and consequent, then processor cache prefetches make most memory reads almost free, but when it is accessed in a random or reverse order, this loses efficiency dramatically.

Second issue is less obvious and has to do with AoS. The secret is there is an alternative, an SoA or *Structure of Arrays*[^16]. In such case the normals would be stored like this:
...
With all these issues I made when working with SIMD as a newcomer, it *technically* did improve performance, but only a slightly bit, so little, it doesn't really matter. SIMDs are very promising though, and as any other tools, have to be used correctly. Follow me through my journey I am going to document in the upcoming articles.
